...
# Versione 3.4 Add a MALAT1 versus % MT protein genes data and plot 

setwd("/scratch")
args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 1) {
  stop("Usage: Rscript Ribomito_v3.3.R <infile> <rownames_format> <kit> [counts_threshold] [out_metrics] [out_freq] [mtx_file] [barcodes_file] [features_file]")
}
...
counts_threshold <- ifelse(length(args) >= 4, as.numeric(args[4]), 2)
out_metrics <- ifelse(length(args) >= 5, args[5], "ribomito_metrics.csv")
out_freq <- ifelse(length(args) >= 6, args[6], "ribomito_freq.csv")
mtx_file <- if (length(args) >= 7) args[7] else NULL
barcodes_file <- if (length(args) >= 8) args[8] else NULL
features_file <- if (length(args) >= 9) args[9] else NULL
...
ribomito_qc <- function(infile,
						    rownames_format = c("ensemblID:symbol", "symbol"),
							  kit = c("scRNAseq", "scMultiomics10X"),
						    counts_threshold = 2,
  ...
  if(kit == "scMultiomics10X"){
    ...
    p = ggplot(out_dt, aes(x=mito.score, y=MALAT1.counts, color=expression.groups)) + 
      geom_point(size=0.8, alpha=0.7, shape = 16) + 
...
#test
ribomito_qc(infile=infile, rownames_format=rownames_format, kit=kit, out_metrics=out_metrics, out_freq=out_freq, 
