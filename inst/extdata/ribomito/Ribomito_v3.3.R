...
if (length(args) < 1) {
  stop("Usage: Rscript Ribomito_v3.3.R <infile> <rownames_format> [counts_threshold] [out_metrics] [out_freq] [mtx_file] [barcodes_file] [features_file]")
}

infile <- ifelse(length(args) >= 1, args[1])
rownames_format <- ifelse(length(args) >= 2, args[2], "symbol")
counts_threshold <- ifelse(length(args) >= 3, as.numeric(args[3]), 2)
out_metrics <- ifelse(length(args) >= 4, args[4], "ribomito_metrics.csv")
out_freq <- ifelse(length(args) >= 5, args[5], "ribomito_freq.csv")
mtx_file <- if (length(args) >= 6) args[6] else NULL
barcodes_file <- if (length(args) >= 7) args[7] else NULL
features_file <- if (length(args) >= 8) args[8] else NULL
...
#test
ribomito_qc(infile=infile, rownames_format=rownames_format, out_metrics=out_metrics, out_freq=out_freq, 
