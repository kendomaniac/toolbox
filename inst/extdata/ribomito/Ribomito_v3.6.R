...
#' @param beads_location The comma separated file which contains the beads locations required for Seeker specific visualisation. For visiumHD this is  the comma separate file of visium_parquet, which contains the association among barcodes as different resolution 0.02, 0.08 and 0.16 um and cellID
...
ribomito_qc <- function(infile,
  ...
  if (use_sparse == TRUE && kit == "visiumHD"){
    # execute bc2cellid
    bc2cellid(beads_location=beads_location, mtx_path=infile)
    #
  ...
  #for visiumHD only
  mito_counts1 <- if (any(mito_idx)) {
    if (use_sparse && kit=="visiumHD") Matrix::colSums(counts_mat1[mito_idx, , drop = FALSE]) else colSums(counts_mat1[mito_idx, , drop = FALSE], na.rm = TRUE)
  } else {
    rep(0, ncol(counts_mat1))
  }
  ...
  #for visiumHD only
  ribo_counts1 <- if (any(ribo_idx)) {
    if (use_sparse && kit=="visiumHD") Matrix::colSums(counts_mat1[ribo_idx, , drop = FALSE]) else colSums(counts_mat1[ribo_idx, , drop = FALSE], na.rm = TRUE)
  } else {
    rep(0, ncol(counts_mat1))
  }
